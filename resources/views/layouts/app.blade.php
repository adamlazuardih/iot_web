            ...
            setInterval(function() {
                $.ajax({
                    ...
                    success: function(response) {
                        const latestSensorsuhu = response.length > 0 ? response[response
                            .length - 1].sensorsuhu : null;
                        console.log('Data sensorsuhu terbaru:', latestSensorsuhu);
                        $('#suhuRealtime').text(latestSensorsuhu !== null ? latestSensorsuhu :
                            '0');
                    },
        ...
        $(document).ready(function() {
            setInterval(function() {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(response) {
                        const latestSensorph = response.length > 0 ? response[response.length -
                            1].sensorph : null;
                        console.log('Data sensorsuhu terbaru:', latestSensorph);
                        $('#phRealtime').text(latestSensorph !== null ? latestSensorph :
                            '0');
                    },
                    error: function(error) {
                        console.error('Error fetching data:', error);
        ...
        $(document).ready(function() {
            setInterval(function() {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(response) {
                        const latestSensoramonia = response.length > 0 ? response[response.length -
                            1].sensoramonia : null;
                        console.log('Data sensorsuhu terbaru:', latestSensoramonia);
                        $('#amoniaRealtime').text(latestSensoramonia !== null ? latestSensoramonia :
                            '0');
                    },
                    error: function(error) {
                        console.error('Error fetching data:', error);
        ...
        $(document).ready(function() {
            setInterval(function() {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(response) {
                        const latestSensortds = response.length > 0 ? response[response.length -
                            1].sensortds : null;
                        console.log('Data sensor tds terbaru:', latestSensortds);
                        $('#tdsRealtime').text(latestSensortds !== null ? latestSensortds :
                            '0');
                    },
                    error: function(error) {
                        console.error('Error fetching data:', error);
        ...
        $(document).ready(function() {
            setInterval(function() {
                $.ajax({
                    ...
                    dataType: 'json',
                    success: function(response) {
                        const latestSensorkeruh = response.length > 0 ? response[response.length -
                            1].sensorkekeruhan : null;
                        console.log('Data sensor keruh terbaru:', latestSensorkeruh);
                        $('#keruhRealtime').text(latestSensorkeruh !== null ? latestSensorkeruh :
                            '0');
                    },
                    error: function(error) {
                        console.error('Error fetching data:', error);
